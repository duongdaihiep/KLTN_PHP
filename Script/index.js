...
async function loadTrainingData() {
    ...
    for (const label of labels) {
        const descriptors = [];
        for (let i = 1; i <= 4; i++) {
        console.log(`Đang tải hình ảnh: ./data/${label}/${i}.jpeg`);
        ...
        if (!detection) {
            console.error(`Không phát hiện khuôn mặt trong hình: ${label}/${i}.jpeg`);
            continue;
        }
        ...
        console.warn(`Không có dữ liệu nào hợp lệ cho label: ${label}`);
        }
    }
...
faceMatcher = new faceapi.FaceMatcher(trainingData, 0.2);
